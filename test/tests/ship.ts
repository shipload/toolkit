...
function createStationaryShip(overrides: ShipOverrides = {}) {
    return Ship.fromState({
        id: 2,
        owner: 'teamgreymass',
        name: 'Stationary Ship',
        location: Coordinates.from({x: Int64.from(5), y: Int64.from(10)}),
        mass: 500000,
...
function createShipWithSchedule() {
    return Ship.fromState({
        id: 3,
        owner: 'teamgreymass',
...
suite('Ship', function () {
    suite('maxDistance', function () {
        test('calculates max distance from capacity/drain', function () {
            const ship = createStationaryShip()
    ...
    suite('hasSchedule', function () {
        test('returns false when ship has no schedule', function () {
            const ship = createStationaryShip()
            assert.isFalse(ship.hasSchedule)
        })

        test('returns true when ship has schedule with tasks', function () {
            const ship = createShipWithSchedule()
            assert.isTrue(ship.hasSchedule)
        })
    })
    ...
    suite('currentLocation', function () {
        test('returns coordinates', function () {
            const ship = createStationaryShip()
            const loc = ship.currentLocation
    ...
    suite('cargo management', function () {
        ...
        test('cargo returns items when ship has cargo', function () {
            ...
            const ship = createStationaryShip({cargo: [cargo1, cargo2]})
        ...
        test('totalCargoMass calculates mass of all cargo', function () {
            ...
            const ship = createStationaryShip({cargo: [cargo]})
    ...
    suite('totalMass', function () {
        ...
        test('includes cargo mass when present', function () {
            const shipWithoutCargo = createStationaryShip()
            const massWithoutCargo = shipWithoutCargo.totalMass
            const cargo = createCargoItem(1, 100, 50)
            const shipWithCargo = createStationaryShip({cargo: [cargo]})
    ...
    suite('availableCapacity', function () {
        ...
        test('returns 0 when full', function () {
            const ship = createStationaryShip({capacity: 600000})
    ...
    suite('setLocation', function () {
        test('sets cached location', function () {
            ...
            assert.equal(ship.locationObject.coordinates.x.toNumber(), 99)
        })
    })
    ...
    suite('needsRecharge', function () {
        ...
        test('returns true when not fully charged', function () {
            const ship = createStationaryShip({energy: 1000})
    ...
    suite('calculateSaleValue', function () {
        ...
        test('skips cargo with zero quantity', function () {
            ...
            const ship = createStationaryShip({cargo: [cargoWithQty, cargoEmpty]})
    ...
    suite('afterSellGoods', function () {
        ...
        test('only affects goods being sold', function () {
            ...
            const ship = createStationaryShip({cargo: [cargo1, cargo2, cargo3]})
