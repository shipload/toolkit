...
import {ITEM_SHIP_T1_PACKED, ITEM_ENGINE_T1} from '../src/data/item-ids'
...
describe('projectEntity (lean row recompute)', () => {
    test('recomputes capabilities from stats + item_id + modules when cap fields absent', () => {
        ...
        const expected = computeEntityCapabilities(
            decodeCraftedItemStats(ITEM_SHIP_T1_PACKED, seed),
            ITEM_SHIP_T1_PACKED,
            modules,
            getEntityLayout(ITEM_SHIP_T1_PACKED)?.slots ?? []
        )
