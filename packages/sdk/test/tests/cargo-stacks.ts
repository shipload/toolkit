...
suite('CargoStack helpers', function () {
    suite('stackKey', function () {
        test('keys by item_id and seed', function () {
            assert.equal(stackKey(stack(5, 10, 42)), '5:42')
        })

        test('treats undefined seed as 0', function () {
            assert.equal(stackKey(stack(5, 10)), '5:0')
        })
    })

    suite('mergeStacks', function () {
        test('appends a new stack with no match', function () {
            const result = mergeStacks([stack(1, 5, 10)], stack(2, 3, 20))
            assert.equal(result.length, 2)
            assert.equal(result[1].item_id.toNumber(), 2)
        })

        test('merges quantity into matching (item_id, seed)', function () {
            const result = mergeStacks([stack(1, 5, 10)], stack(1, 7, 10))
            assert.equal(result.length, 1)
            assert.equal(result[0].quantity.toNumber(), 12)
        })

        test('keeps separate stacks for different seeds', function () {
            const result = mergeStacks([stack(1, 5, 10)], stack(1, 7, 20))
            assert.equal(result.length, 2)
        })

        test('merges by item_id when both stacks have undefined seed', function () {
    ...
    suite('removeFromStacks', function () {
        test('decrements quantity from matching stack', function () {
            const result = removeFromStacks([stack(1, 10, 5)], stack(1, 3, 5))
            assert.equal(result.length, 1)
            assert.equal(result[0].quantity.toNumber(), 7)
        })

        test('removes stack when quantity reaches zero', function () {
            const result = removeFromStacks([stack(1, 10, 5)], stack(1, 10, 5))
            assert.equal(result.length, 0)
        })

        test('throws INSUFFICIENT_ITEM_QUANTITY on underflow', function () {
            assert.throws(
        ...
        test('throws INSUFFICIENT_ITEM_QUANTITY when stack absent', function () {
            assert.throws(
    ...
    suite('stacksEqual', function () {
        test('true for identical stacks', function () {
            assert.isTrue(stacksEqual(stack(1, 5, 10), stack(1, 5, 10)))
        })

        test('false on quantity mismatch', function () {
            assert.isFalse(stacksEqual(stack(1, 5, 10), stack(1, 6, 10)))
        })

        test('false on seed mismatch', function () {
