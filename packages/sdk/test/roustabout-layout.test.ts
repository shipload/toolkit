...
test('Roustabout support slots operate at 80 percent', () => {
    expect(getEntityLayout(ITEM_ROUSTABOUT_T1A_PACKED)?.slots).toEqual([
...
test('Roustabout upgrades use 85 percent engine and power-core slots', () => {
    const directChildIds = eligibleUpgrades(ITEM_ROUSTABOUT_T1A_PACKED).map(
...
test('Prospector T1 upgrades into Prospector T2 at 90 percent engine and power-core slots', () => {
    const childIds = eligibleUpgrades(ITEM_PROSPECTOR_T1A_PACKED).map(
        (recipe) => recipe.outputItemId
    )
    expect(childIds).toEqual([ITEM_PROSPECTOR_T2A_PACKED])

    const slots = getEntityLayout(ITEM_PROSPECTOR_T2A_PACKED)?.slots
...
test('Prospector T2 has a single power core, engine, and gatherer slot', () => {
    expect(getEntityLayout(ITEM_PROSPECTOR_T2A_PACKED)?.slots).toEqual([
