...
import {ITEM_HUB_T2_PACKED, ITEM_SHIP_T1_PACKED, ITEM_WAREHOUSE_T2_PACKED} from '@shipload/sdk'
...
describe('resolveDeploySlot', () => {
    test('structure with a cell returns the slot', () => {
        expect(resolveDeploySlot(ITEM_WAREHOUSE_T2_PACKED, {hub: 5n, gx: -1, gy: 0})).toEqual({
            hub: 5n,
            gx: -1,
            gy: 0,
        })
    ...
    test('structure without a cell throws', () => {
        expect(() => resolveDeploySlot(ITEM_WAREHOUSE_T2_PACKED, undefined)).toThrow(
            /requires a hub cell/
        )
    })
    test('hub with a cell throws', () => {
        expect(() => resolveDeploySlot(ITEM_HUB_T2_PACKED, {hub: 5n, gx: 0, gy: 0})).toThrow(
            /does not take a hub cell/
        )
    })
    test('hub without a cell returns undefined', () => {
        expect(resolveDeploySlot(ITEM_HUB_T2_PACKED, undefined)).toBeUndefined()
    })
